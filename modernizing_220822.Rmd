...
I am running GRASS 8.2.0 and R 4.2.1, with recent `update.packages()`. Please install a recent GRASS https://grass.osgeo.org/download and a recent R https://cran.r-project.org/ before reaching Florence. 
...
I will try to ensure that a CRAN macOS M1 binary of **rgrass** is available before we meet, source install on M! works, but a vignette is broken.
...
As **rgdal** will retire soon, using **rgdal** in the **raster** package [@raster] is not a feasible way to coerce a `"SpatRaster"` object to a `"RasterLayer"` object. 
...
We can use a temporary file in the `"RRASTER"` format that `terra::writeRaster()` can write using GDAL, and `raster::raster()` can read using R code rather than GDAL:
...
Next we rasterize the negative buffered buildings using `bbo-r` created above:
...
There are clearly numerous, and hopefully this session will both define others, and maybe recruit contributors to ones already identified.
